...
当前状态：**主 Arena 已达到本地部署测试条件。** Stage 0~5 MVP 已完成；Stage 6 已完成跨平台/ARM64/1-CPU smoke、Piper/Kokoro/MeloTTS/CosyVoice 真实 CPU gate、扩展模型 external Python worker，以及 Qwen3-TTS 0.6B CustomVoice 官方 `qwen-tts` CPU Adapter、本地资产准备和真实 hosted CPU synthesis gate。Qwen3 官方 FP32 功能路径已验证，但当前实测 RTF/内存仍不符合弱设备优化目标，因此继续 `experimental + disabled`。
...
- Qwen3-TTS 0.6B CustomVoice：官方 CPU Adapter + local snapshot manifest + hosted real CPU synthesis ✅；quantization / low-memory route ⏳
...
Arena 只接官方 `Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice`，不把 Base voice-clone checkpoint 混入现有 `text + voice id` 请求合同。当前功能基线是官方 `qwen-tts==0.1.1` + CPU FP32；Arena 暂不宣称 speed、seed、streaming 或 voice-clone capability。
...
首次真实 hosted gate（GitHub-hosted Ubuntu x86_64 / Python 3.11 / 2 threads）已通过：9.04s 音频，48.05s inference，RTF 5.315，peak RSS 5317.8MB。该结果用于证明官方 CPU 路径可运行，不代表端侧性能已达标。

`.github/workflows/qwen3-real-cpu-gate.yml` 可通过 `.github/model-gates/qwen3.trigger` 显式触发真实重模型 gate；普通业务代码 push 不会下载约 2.5GB 模型资产。`extended-model-gates.yml` 仍保留 manual dispatch 入口。
