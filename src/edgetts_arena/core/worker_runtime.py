...
def run_isolated_model(task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        adapter = factory()
        adapter.load_model(str(task.get("model_path") or ""), device="cpu", num_threads=int(task["num_threads"]))
        output, metrics = MetricsCollector().measure_inference(adapter, str(task["text"]), **dict(task.get("infer_kwargs") or {}))
        write_wav(str(task["audio_path"]), output.audio, output.sample_rate)
        return {"status": "success", "metrics": metrics.to_dict(), "metadata": output.metadata, "error": None}
    except Exception as exc:
        return {"status": "error", "metrics": None, "metadata": None, "error": _normalize_worker_error(exc)}
    finally:
...
def run_isolated_repeated_model(task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "measurements": measurements, "metadata": metadata, "audio_written": audio_written, "error": None}
    except Exception as exc:
        return {"status": "error", "measurements": [], "metadata": None, "audio_written": False, "error": _normalize_worker_error(exc)}
    finally:
        if adapter is not None:
            try:
                adapter.unload_model()
